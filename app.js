...
function createArc(x, y, r, color) {
  ctx.fillStyle = color;
  ctx.beginPath();
...
function render() {
  createRect(0, 0, canvas.width, canvas.height, "#152D35");
  createText(user.score, canvas.width / 4, canvas.height / 5);
  createText(computer.score, (3 * canvas.width) / 4, canvas.height / 5);
  createNet();
  ...
  createArc(ball.x, ball.y, ball.radius, ball.color);
}
function game() {
  update();
  render();
